/* ==========================================================================
   LÓGICA AIA - VERSÃO FINAL INTEGRADA (PROVAS ANTIGAS 2025-2021)
   ========================================================================== */
...
/**
 * 2. ESTRUTURA DE DISCIPLINAS (Anos 2021-2025)
 */
const ESTRUTURA_MED = {
    famema: {
        labelSims: ['2025', '2024', '2023', '2022', '2021'],
...
function gerarTabelasHibridas() {
    FACULDADES.forEach(fac => {
        ...
        const labels = isMed ? ESTRUTURA_MED[fac].labelSims : ['2025', '2024', '2023', '2022', '2021'];
    ...
    document.addEventListener('input', (e) => {
        if (e.target.classList.contains('input-paulista')) {
            ...
            if (FACULDADES_MED.includes(fac)) {
                calcularMedicina(row, fac);
            } else {
                calcularLinhaPadrao(row);
            }
...
function calcularLinhaPadrao(row) {
    ...
    if(percEl) {
        ...
        percEl.style.color = p < 50 ? '#ef4444' : '#05c46b';
    }
}
...
function criarChart(canvasId, labels, fac) {
    ...
    if (!ctx) return null;
    const labelsLegenda = ['2025', '2024', '2023', '2022', '2021'];
    const cores = ['#00b8d4', '#05c46b', '#ff6b6b', '#ffa502', '#3742fa'];

    return new Chart(ctx, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: labelsLegenda.map((l, i) => ({ label: l, data: [], backgroundColor: cores[i] }))
        },
        options: {
            responsive: true,
            maintainAspectRatio: false, // CORREÇÃO PARA O GRÁFICO PREENCHER OS 550PX
            layout: {
                padding: { bottom: 30 } // ESPAÇO PARA OS NOMES DAS MATÉRIAS
            },
            scales: { 
                y: { 
                    beginAtZero: true, 
...
function configurarAbas() {
    document.querySelectorAll('.btn-aba-faculdade').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // FORÇA O RESIZE IMEDIATO DO GRÁFICO AO TROCAR DE ABA
            setTimeout(() => {
